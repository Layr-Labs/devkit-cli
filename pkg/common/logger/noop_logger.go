...
// Actor-based methods - for testing, we'll log these with actor prefix
func (l *NoopLogger) TitleWithActor(actor iface.Actor, msg string, args ...any) {
	formatted := fmt.Sprintf("\n"+msg+"\n", args...)
	l.addEntry("TITLE", fmt.Sprintf("[%s] %s", string(actor), formatted))
}

func (l *NoopLogger) InfoWithActor(actor iface.Actor, msg string, args ...any) {
	msg = strings.Trim(msg, "\n")
	if msg == "" {
		return
	}
	formatted := fmt.Sprintf(msg, args...)
	l.addEntry("INFO", fmt.Sprintf("[%s] %s", string(actor), formatted))
}

func (l *NoopLogger) WarnWithActor(actor iface.Actor, msg string, args ...any) {
	msg = strings.Trim(msg, "\n")
	if msg == "" {
		return
	}
	formatted := fmt.Sprintf(msg, args...)
	l.addEntry("WARN", fmt.Sprintf("[%s] %s", string(actor), formatted))
}

func (l *NoopLogger) ErrorWithActor(actor iface.Actor, msg string, args ...any) {
	msg = strings.Trim(msg, "\n")
	if msg == "" {
		return
	}
	formatted := fmt.Sprintf(msg, args...)
	l.addEntry("ERROR", fmt.Sprintf("[%s] %s", string(actor), formatted))
}

func (l *NoopLogger) DebugWithActor(actor iface.Actor, msg string, args ...any) {
	msg = strings.Trim(msg, "\n")
	if msg == "" {
		return
	}
	formatted := fmt.Sprintf(msg, args...)
	l.addEntry("DEBUG", fmt.Sprintf("[%s] %s", string(actor), formatted))
}
