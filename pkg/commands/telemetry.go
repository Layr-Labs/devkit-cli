...
func displayGlobalTelemetryStatus(logger iface.Logger, prefix string) error {
	...
	if globalPreference == nil {
		logger.InfoWithActor(iface.ActorTelemetry, "%s: Not set (defaults to disabled)", prefix)
	} else if *globalPreference {
		logger.InfoWithActor(iface.ActorTelemetry, "%s: Enabled", prefix)
	} else {
		logger.InfoWithActor(iface.ActorTelemetry, "%s: Disabled", prefix)
	}
	return nil
}
...
func showTelemetryStatus(logger iface.Logger, global bool) error {
	...
	if projectErr == nil && projectSettings != nil {
		if effectivePreference {
			logger.InfoWithActor(iface.ActorTelemetry, "Telemetry: Enabled (project setting)")
		} else {
			logger.InfoWithActor(iface.ActorTelemetry, "Telemetry: Disabled (project setting)")
		}
		...
		if effectivePreference {
			logger.InfoWithActor(iface.ActorTelemetry, "Telemetry: Enabled (global setting)")
		} else {
			logger.InfoWithActor(iface.ActorTelemetry, "Telemetry: Disabled (global setting)")
		}
	}
...
func enableTelemetry(logger iface.Logger, global bool) error {
	if global {
		...
		logger.InfoWithActor(iface.ActorTelemetry, "✅ Global telemetry enabled")
		logger.InfoWithActor(iface.ActorTelemetry, "New projects will inherit this setting.")
		return nil
	}
	...
	logger.InfoWithActor(iface.ActorTelemetry, "✅ Telemetry enabled for this project")
	return nil
}
...
func disableTelemetry(logger iface.Logger, global bool) error {
	if global {
		...
		logger.InfoWithActor(iface.ActorTelemetry, "❌ Global telemetry disabled")
		logger.InfoWithActor(iface.ActorTelemetry, "New projects will inherit this setting.")
		return nil
	}
	...
	logger.InfoWithActor(iface.ActorTelemetry, "❌ Telemetry disabled for this project")
	return nil
}
